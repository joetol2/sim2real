...
const loadedScripts2 = new Set<string>();
...
function SimCanvas2() {
  ...
  useEffect(() => {
    ...
    const loadScript = (src: string): Promise<void> =>
      new Promise((resolve) => {
        if (loadedScripts2.has(src)) { resolve(); return; }
        const script = document.createElement("script");
        script.src = src;
        script.onload = () => { loadedScripts2.add(src); resolve(); };
        script.onerror = () => { console.warn("fizx2: failed to load", src); resolve(); };
    ...
    const scripts = [
      ...
      `${base}fizx2/ball23.js?v=${v}`,
      `${base}fizx2/delaunay.js?v=${v}`,
      `${base}fizx2/display_canvas.js?v=${v}`,
      `${base}fizx2/test.js?v=${v}`,
    ];
